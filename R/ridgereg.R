...
ridgereg <- setRefClass("ridgereg",
                        ...
                        methods = list(
                          initialize = function(data, formula, lambda) {
                            ...
                            beta_hat <<- solve(t(X) %*% X + diag(lambda, ncol(X))) %*% t(X) %*% y
                            # Unscale regression coefficients
                            beta_hat[1] <<- beta_hat[1] * attr(y, "scaled:scale") + attr(y, "scaled:center") - sum(sapply(1:(length(beta_hat)-1), function(x) {
                              attr(X, "scaled:center")[-1][x] * attr(y, "scaled:scale") * beta_hat[x+1] / attr(X, "scaled:scale")[-1][x]
                            }))
                            beta_hat[2:length(beta_hat)] <<- sapply(1:(length(beta_hat)-1), function(x) {
                          ...
                          predict = function(object, newdata) {
                            ...
                            new_X <- data.frame(rep(1, nrow(newdata)),
